(function(){
    ...
    AppExts = function(app) {
        this._app = app    
        this._loading = {}
        //this._basesrc = this._app.nav.basepath+'xio/sdk/ext/'
    ...
    AppRoutes.prototype.getHandler = function (path) {
        ...
        $("page[xio-path]").each( function() {
            routes[ $(this).attr('path') ] = {
                'target': $(this)
            }
        })
        $("section[xio-path]").each( function() {
            routes[ $(this).attr('path') ] = {
                'target': $(this)
            }
        })
	...
	XioWebApp = function(endpoint,handler) {
    ...
    XioWebApp.prototype.init = function () {
    ...
    XioWebApp.prototype.run = function () {
    
        this.log.info('xio run')
        ...
        if (!this.user.id) {
            this.user = XioUser.login(null,prompt('seed ?'))
        }

        // connect
        this.user.connect( this.server.http )
    ...
    XioWebApp.prototype.load = function (src,callback) {
    ...
    XioWebApp.prototype.template = function (template,data) {
    ...
    /*
    XioWebApp.prototype.login = function (seed,password) {
        // init user
    ...
    XioWebApp.prototype.logout = function (callback) {
    ...
    */
    ...
    XioWebApp.prototype.ready = function (callback) {
        if (this._ready) {
            callback()
        } else {
    ...
    XioWebApp.prototype.publish = function (topic,data) {
    ...
    XioWebApp.prototype.subscribe = function (topic,callback) {
        console.log('subscribe '+topic,callback)
        if (!this._events[topic])
            this._events[topic] = []     
        this._events[topic].push(callback)
    }
    XioWebApp.prototype.on = function (topic,callback) {
    ...
    XioWebApp.prototype.render = function (path,data) {
...
$(window).bind( 'hashchange', function(e) { 
    ...
    if (hash.startsWith(app.nav.hashbang)) {
        ...
        // simple section@id nav 
        app.render('/'+hash.slice('1'))    
    }
});
